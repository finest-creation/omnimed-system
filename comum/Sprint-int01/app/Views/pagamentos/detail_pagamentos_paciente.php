...
$usuarioComum = $usuarioComumModel->find($id);

$pagamentos = new Pagamentos();
$pagamentosModel = new PagamentosModel();

$pagamentos = $pagamentosModel->historicoPaciente($id);
        ...
        <section id="counts" class="counts">
            <div class="container">
                <div class="section-title margem">
                    ...
                    <p>Paciente: <?php echo $usuarioComum->USC_NOME;  ?><br />CPF: <?php echo $usuarioComum->USC_CPF; ?></p>
                </div>

                <a href="/pagamentos/pacientes/" class="appointment-btn">Voltar</a>
                <br />
                <br />
                                ...
                                <table id="example" class="table table-striped table-bordered grid" style="width:100%;">
                                    ...
                                    <tbody>
                                        <?php foreach ($pagamentos as $key => $pagamento) { ?>
                                            <tr style="text-align: center;">
                                                <td><?php if (date('Y-m-d') > date($pagamento->PGD_VENCIMENTO) && $pagamento->PGD_DATA_PAGAMENTO == null) {
                                                        echo '<p style="color: red;">' . $pagamento->PGD_VENCIMENTO . '</p>';
                                                    } else {
                                                        echo $pagamento->PGD_VENCIMENTO;
                                                    } ?></td>
                                                <td><?php if ($pagamento->PGD_DATA_PAGAMENTO == null) {
                                                        echo 'Em aberto';
                                                    } else {
                                                        echo $pagamento->PGD_DATA_PAGAMENTO;
                                                    } ?></td>
                                                <td><?php echo $pagamento->PGD_VALOR_TOTAL; ?></td>
                                                <td><?php echo $pagamento->PGD_QUANTIDADE_DEPENDENTES; ?></td>
                                                <td><?php echo $pagamento->PGD_VALOR_PLANO; ?></td>
                                            </tr>
                                        <?php } ?>
                                    </tbody>
                                </table>
